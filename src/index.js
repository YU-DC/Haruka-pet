...
/* Class Definition */
class Haruka {
  constructor({ x, y, speed, actor, isAngry }) {
    this.selector = '.actor-haruka.' + actor
  ...
  generateActor({ x, y }) {
    ...
    $(img)
      .addClass(`actor-haruka draggable ${this.actor}`)
      .css({ left: x, top: y })
    $('body').append(img)
  ...
  updateStatus(status) {
    if (status === 'rand') {
      status = 'interact_' + randInt(1, 9).toString()
    }
    if (status === 'angry') {
  ...
  async sendMessage(content) {
    await this.getRandMessage(this.actor).then((message) => {
      ...
      $(div)
        .addClass(`message-box-haruka ${this.actor} ${timeStamp}`)
        .css({
          left: this.x + 100,
          top: this.y + 50
        })
  ...
  async removeMessage(actorName, timeStamp) {
    if (timeStamp === undefined) {
      $(`.message-box-haruka.${actorName}`).remove()
    } else {
      $(`.message-box-haruka.${actorName}.${timeStamp}`).remove()
    }
  }
...
const Interactions = {
  ...
  followClick: function (actor) {
    $(document).mousedown((e) => {
      $('.bait-haruka').remove() // only one candy appear
...
async function main() {
  ...
  await loadStorage('CONFIG').then((config) => {
    ...
    Object.keys(TABLE).forEach((actorName) => {
      const liveurl = new RegExp('/' + config.live + '/(\\d+)(\\?)')
      console.log(liveurl)
      const roomid = new RegExp(config.actors[actorName].url) 
      let isAngry = false
      if (liveurl.test(url)) {
        if ( !roomid.test(url)) {
          isAngry = true
